...
// Інтерфейс для відповіді getUserMessages, якщо потрібно (можна винести в types)
interface MessageWithUserDetails {
  messageID: number;
  senderID: number;
  receiverID: number;
  message: string;
  messageFileContentUrl: string | null; // Зберігає ім'я файлу або null
  send_at: Date;
 ...
 */
const getMessagesForUser = async (userId1: number, userId2: number): Promise<MessageWithUserDetails[]> => {
  if (!userId1 || !userId2) {
  ...
  try {
    ...
    `);
    return result.recordset as MessageWithUserDetails[];
  } catch (error) {
...
): Promise<MessageWithUserDetails | null> => { 
  ...
  try {
    ...
    try {
        const request = transaction.request()
          ...
          .input('message', sql.NVarChar, message || null) // Дозволяємо null, якщо є тільки файл
          .input('messageFileContentUrl', sql.NVarChar, messageFilename || null) // Зберігаємо ім'я файлу
          .input('created_at', sql.DateTime, new Date()); // Використовуємо GETDATE() або new Date()
        ...
        if (result.recordset && result.recordset.length > 0) {
            ...
            const senderDataResult = await userRequestSender.input('senderIdParam', sql.Int, newMessage.senderID)
            ...
            const receiverDataResult = await userRequestReceiver.input('receiverIdParam', sql.Int, newMessage.receiverID)
    ...
    throw new Error(`Database error while adding message: ${(error as Error).message}`);
  }
};

const deleteMessageFromDB = async (messageID: number, currentUserId: number): Promise<boolean> => {
  ...
  try {
    ...
    if (result.rowsAffected[0] === 0) {
      const checkRequest = pool.request().input('checkMessageID', sql.Int, messageID);
...
const getChatsForUser = async (userId: number): Promise<ChatDetails[]> => {
  if (!userId) {
  ...
  try {
    ...
    `);
    return result.recordset as ChatDetails[];
  } catch (error) {
...
const updateMessageInDB = async (
  ...
  newText?: string,
  newMessageFilename?: string | null // undefined - не чіпати, null - видалити, string - нове
): Promise<{ oldFilename: string | null, updatedMessage: MessageWithUserDetails | null }> => {
  const pool = await connectDB();
  const transaction = pool.transaction();
  ...
  try {
    ...
    const request = transaction.request().input("messageID", sql.Int, messageID);

    const checkResult = await request.query(
      `SELECT sender_id, message_file_content_url FROM messages WHERE message_id = @messageID`
    );
    if (checkResult.recordset.length === 0) {
      throw new Error("Message not found");
    }
    ...
    const updateRequest = transaction.request().input("messageIdForUpdate", sql.Int, messageID);

    if (newText !== undefined) {
      updateFields.push("message = @newText");
      updateRequest.input("newText", sql.NVarChar, newText);
    }

    if (newMessageFilename !== undefined) {
      updateFields.push("message_file_content_url = @newMessageFilename");
      updateRequest.input("newMessageFilename", sql.NVarChar, newMessageFilename); // newMessageFilename може бути null
    }
    ...
    if (transactionBegun && transaction) {
      try { await transaction.rollback(); } catch (rbErr) { console.error("Rollback error in updateMessageInDB:", rbErr); }
    }
...
// Допоміжна функція для отримання одного повідомлення (потрібна для повернення оновленого)
 const getSingleMessageById = async (messageID: number): Promise<MessageWithUserDetails | null> => {
    try {
        const pool = await connectDB();
        ...
        if (result.recordset.length > 0) {
            return result.recordset[0] as MessageWithUserDetails;
        }
        return null;
    } catch (error) {
...
export { getMessagesForUser, addMessage, deleteMessageFromDB, getChatsForUser, updateMessageInDB, getSingleMessageById };