...
import sql, { Transaction as MSSQLTransaction } from 'mssql';
import { connectDB } from '../config/db';
import { CommentWithAuthorData, FullPostData, PopularPostData } from '../types/postTypes'; // Імпортуємо наш тип

export const getPostsFromDB = async (
  limit: number = 10,
  offset: number = 0,
  userIdForFeed?: number,
  currentAuthUserId?: number
): Promise<{ posts: FullPostData[], totalCount: number }> => {
  try {
    ...
    if (userIdForFeed !== undefined) {
      ...
      countRequest.input("userIdForFeedParam_count", sql.Int, userIdForFeed);
    }
    ...
    if (userIdForFeed !== undefined) {
        request.input("userIdForFeedParam_select", sql.Int, userIdForFeed);
    }
    ...
    const posts = result.recordset.map((post: any): FullPostData => ({
        postId: post.postId, 
        userId: post.userId, 
        title: post.title, 
...
export const getPostsByUserIdFromDB = async (
  userIdForPosts: number,
  limit: number = 10,
  offset: number = 0,
  currentAuthUserId?: number
): Promise<{ posts: FullPostData[], totalCount: number }> => {
    ...
    try {
        const pool = await connectDB();
        const countRequest = pool.request().input("userIdForPostsCount", sql.Int, userIdForPosts);
        const countResult = await countRequest.query(
          `SELECT COUNT_BIG(*) as totalCount FROM posts WHERE user_id = @userIdForPostsCount`
        );
        ...
        const { posts } = await getPostsFromDB(limit, offset, userIdForPosts, currentAuthUserId);
        return { posts, totalCount };
...
export const addPostToDB = async (
  userID: number, title: string, content: string, contentImgFilename: string | null = null
): Promise<{ postId: number; message: string }> => {
  if (!userID || !title || !content) { throw new Error("❌ Missing required fields: userID, title, and content"); }
  const pool = await connectDB();
  let transaction: sql.Transaction | undefined = pool.transaction();
  try {
    ...
    const request = transaction.request()
      ...
      .input("contentImgFilename", sql.NVarChar, contentImgFilename || null);
    ...
    await postDataRequest.input("postIDParam", sql.Int, postId).query(`
    ...
    if (transaction) { try { await transaction.rollback(); } catch (rbError) { console.error("Rollback error in addPostToDB:", rbError); }}
    if (error instanceof Error) { console.error("❌ Error adding post to DB:", error.message); }
    else { console.error("❌ Error adding post to DB:", error); }
 ...
 */
export const deletePostFromDB = async ( postId: number, currentUserId: number, userRole: string ): Promise<void> => {
  if (!postId || !currentUserId) { throw new Error("❌ Missing required field: postId or currentUserId"); }
  const pool = await connectDB();
  let transaction: sql.Transaction | undefined = pool.transaction();
  try {
    await transaction.begin();
    const request = transaction.request().input("postIdParam", sql.Int, postId);

    if (userRole !== "admin") {
      request.input("currentUserIdParam", sql.Int, currentUserId);
      const postOwnerResult = await request.query( "SELECT user_id FROM posts WHERE post_id = @postIdParam" );
      if (postOwnerResult.recordset.length === 0) { throw new Error("⚠ Post not found"); }
    ...
    if (transaction) { try { await transaction.rollback(); } catch (rbError) { console.error("Rollback error in deletePostFromDB:", rbError); }}
 ...
 */
export const updatePostFromDB = async (
  postId: number,
  currentUserId: number,
  userRole: string,
  title?: string,
  content?: string,
  newImageFilename?: string | null // undefined - не чіпати зображення, null - видалити зображення, string - нове ім'я
): Promise<string | null> => {
  const pool = await connectDB();
  const transaction: sql.Transaction = pool.transaction(); // Явно вказуємо тип
  ...
  try {
    ...
    let currentMediaUrlFromDB: string | null = null;
    const checkRequest = transaction.request().input("postIdForCheck", sql.Int, postId);
    ...
    const updateRequest = transaction.request().input("postIdForUpdate", sql.Int, postId);

    if (title !== undefined) {
      updateFields.push("label = @titleParam");
      updateRequest.input("titleParam", sql.NVarChar, title);
    }
    if (content !== undefined) {
      updateFields.push("text = @contentParam");
      updateRequest.input("contentParam", sql.NVarChar, content);
    }
    ...
    if (newImageFilename !== undefined) {
      ...
      updateRequest.input("imgParam", sql.NVarChar, newImageFilename); // newImageFilename може бути null
    }
    ...
    if (transactionBegun) { // Відкочуємо, тільки якщо транзакція була розпочата
        try {
            console.warn(`[postModel] Error during post update for post ${postId}, attempting rollback... Error: ${(error as Error).message}`);
 ...
 */
export const searchPostsInDB = async ( 
  searchQuery: string, 
  limit: number = 10,
  offset: number = 0,
  currentAuthUserId?: number 
): Promise<{ posts: FullPostData[], totalCount: number }> => {
  if (!searchQuery || searchQuery.trim() === "") { return { posts: [], totalCount: 0 }; }
  try {
    const pool = await connectDB();
    const countRequest = pool.request().input("queryParamCount", sql.NVarChar, `%${searchQuery}%`);
    const countResult = await countRequest.query(
      `SELECT COUNT_BIG(*) as totalCount FROM posts p WHERE (p.text LIKE @queryParamCount OR p.label LIKE @queryParamCount)`
    );
    const totalCount = Number(countResult.recordset[0].totalCount);
 ...
 */
export const addLikeToPostInDB = async (
  postId: number,
  userId: number
): Promise<boolean> => {
  ...
  try {
    ...
    if (transaction) { 
        try { await transaction.rollback(); console.log("Transaction rolled back in addLikeToPostInDB (error or duplicate)."); } 
        catch (rollbackError) { console.error("Error rolling back transaction in addLikeToPostInDB:", rollbackError); }
    }
    ...
    console.error(
      `❌ Error adding like to post ${postId} by user ${userId}:`,
      error
    );
    return false;
  }
 ...
 */
export const removeLikeFromPostInDB = async (
  postId: number,
  userId: number
): Promise<boolean> => {
  let transaction: sql.Transaction | undefined;
  try {
    ...
    if (result.rowsAffected[0] > 0) {
      ...
      console.warn(
        `User ${userId} had not liked post ${postId} or like already removed.`
      );
      return false;
    }
  } catch (error) {
    if (transaction) {
        try { await transaction.rollback(); console.log("Transaction rolled back in removeLikeFromPostInDB due to error."); } 
        catch (rollbackError) { console.error("Error rolling back transaction in removeLikeFromPostInDB:", rollbackError); }
    }
    console.error(
      `❌ Error removing like from post ${postId} by user ${userId}:`,
      error
    );
    return false;
  }
...
export const getCommentsByPostIdFromDB = async (postId: number): Promise<CommentWithAuthorData[]> => {
  try {
    const pool = await connectDB();
    ...
    return result.recordset as CommentWithAuthorData[];
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
...
): Promise<CommentWithAuthorData | null> => {
  let transaction;
  try {
    const pool = await connectDB();
    transaction = pool.transaction();
    ...
    if (parentCommentId !== undefined && parentCommentId !== null) {
      ...
      request.input('parentCommentId', sql.Int, null); // Явно передаємо NULL
    }
    ...
    await updatePostDataRequest
        .input('postIdForCount', sql.Int, postId)
        .query(`
            UPDATE post_data 
    ...
    const userResult = await userRequest
                            .input('userIdParamForComment', sql.Int, userId)
    ...
    if (transaction) {
        try { await transaction.rollback(); } catch (rbErr) { console.error("Rollback error in addCommentToDB:", rbErr); }
    }
...
export const savePostForUserInDB = async (userId: number, postId: number): Promise<boolean> => {
  try {
    const pool = await connectDB();
    await pool.request()
      .input('userId', sql.Int, userId)
 ...
 */
export const unsavePostForUserInDB = async (userId: number, postId: number): Promise<boolean> => {
  try {
    const pool = await connectDB();
    const result = await pool.request()
      .input('userId', sql.Int, userId)
 ...
 */
export const getSavedPostsForUserFromDB = async (
  targetUserId: number,
  currentAuthUserIdForLikes: number, // Цей параметр потрібен для isLikedByCurrentUser
  limit: number = 10, // Додаємо параметри пагінації
  offset: number = 0
): Promise<{ posts: FullPostData[], totalCount: number }> => {
  ...
  try {
    ...
    const countResult = await countRequest.query(
      `SELECT COUNT_BIG(*) as totalCount FROM saved_posts WHERE user_id = @targetUserIdCount`
    );
    const totalCount = Number(countResult.recordset[0].totalCount);
    ...
    const request = pool.request()
      ...
      .input('currentAuthUserIdParam', sql.Int, currentAuthUserIdForLikes)
...
export const getFollowingPostsFeedFromDB = async (
  ...
  try {
    ...
    const countRequest = pool.request().input("currentAuthUserIdCount", sql.Int, currentAuthUserId);
    const countResult = await countRequest.query(`
...
export const getPopularPostsFromDB = async (
  limit: number = 5, // Змінено дефолтний ліміт для популярних
  offset: number = 0,
  currentAuthUserId?: number
): Promise<{ posts: FullPostData[], totalCount: number }> => {
  try {
...
export const getSinglePostByIdFromDB = async (postId: number, currentAuthUserId?: number): Promise<FullPostData | null> => {
  try {
    const pool = await connectDB();
    const request = pool.request().input("postIdParam", sql.Int, postId);
...
export const updatePostImageInDB = async (postId: number, newImageFilename: string | null): Promise<string | null> => {
  let oldImageFilename: string | null = null;
  const pool = await connectDB();
  ...
  try {
    ...
    const selectRequest = transaction.request().input("postIdSelect", sql.Int, postId);
    const resultSelect = await selectRequest.query(
      `SELECT media_url FROM posts WHERE post_id = @postIdSelect`
    );
    ...
    const updateRequest = transaction.request()
      .input("postIdUpdate", sql.Int, postId)
      .input("newImageFilename", sql.NVarChar, newImageFilename); // Може бути null, якщо зображення видаляється

    const resultUpdate = await updateRequest.query(
      `UPDATE posts SET media_url = @newImageFilename WHERE post_id = @postIdUpdate`
    );
...
export const recursivelyDeleteCommentsAndCount = async (
  commentId: number,
  transaction: MSSQLTransaction // Використовуємо імпортований тип MSSQLTransaction
): Promise<number> => {
  let deletedCount = 0;

  // Знаходимо всі прямі відповіді на цей коментар
  const repliesResult = await transaction.request()
    .input('parentCommentIdParam', sql.Int, commentId)
  ...
  for (const reply of repliesResult.recordset) {
    ...
    deletedCount += await recursivelyDeleteCommentsAndCount(reply.comment_id, transaction);
  }

  // Видаляємо сам коментар (батьківський або поточну відповідь)
  const deleteResult = await transaction.request()
    .input('commentIdToDelete', sql.Int, commentId)
...
export const deleteCommentFromDB = async (
  ...
  userRole: string,
  postId: number
): Promise<{ success: boolean, deletedCommentsCount: number }> => {
  const pool = await connectDB();
  const transaction = pool.transaction();
  ...
  try {
    ...
    const commentCheckRequest = transaction.request()
      .input('commentIdParam', sql.Int, commentId);

    const commentCheckResult = await commentCheckRequest.query(
      `SELECT user_id FROM comments WHERE comment_id = @commentIdParam`
    );
    ...
    const totalDeletedComments = await recursivelyDeleteCommentsAndCount(commentId, transaction);
    ...
    const updatePostDataRequest = transaction.request()
        .input('postIdForCountUpdate', sql.Int, postId)
        .input('deletedCountParam', sql.Int, totalDeletedComments);
    await updatePostDataRequest.query(
        `UPDATE post_data SET comments = comments - @deletedCountParam WHERE post_id = @postIdForCountUpdate AND comments >= @deletedCountParam;
