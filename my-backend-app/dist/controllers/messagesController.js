...
exports.updateMessage = exports.getUserChats = exports.deleteMessage = exports.addNewMessage = exports.getUserMessages = void 0;
const messageModel_1 = require("../models/messageModel");
const websocketService_1 = require("../services/websocketService"); // Переконайтесь, що шлях правильний
const path_1 = __importDefault(require("path"));
const promises_1 = __importDefault(require("fs/promises"));
const getUserMessages = async (req, // Використовуємо Request, якщо req.user типізовано глобально
res, next) => {
    const userId = req.user?.userID;
    const partnerIdParam = req.query.partnerId; // Приймаємо ID співрозмовника
    if (!userId) {
    ...
    try {
        ...
        const messages = await (0, messageModel_1.getMessagesForUser)(userId, partnerId);
        res.json(messages);
    }
    catch (error) {
        next(error);
...
const getUserChats = async (req, // Використовуємо Request
res, next) => {
    const userId = req.user?.userID;
    if (!userId) {
        res.status(401).json({ error: "Unauthorized: User ID not found in token." });
    ...
    try {
        const chats = await (0, messageModel_1.getChatsForUser)(userId);
        res.json(chats);
    }
    catch (error) {
        next(error);
...
const addNewMessage = async (req, // Використовуємо Request
res, next) => {
    ...
    try {
        ...
        const createdMessage = await (0, messageModel_1.addMessage)(loggedInUserId, receiverIdNum, message || '', // Передаємо порожній рядок, якщо повідомлення немає
        ...
        if (createdMessage) {
            ...
            (0, websocketService_1.sendMessageToUser)(loggedInUserId, createdMessage); // Відправляємо собі для оновлення UI
            (0, websocketService_1.sendMessageToUser)(receiverIdNum, createdMessage); // Відправляємо отримувачу
...
const deleteMessage = async (req, // Повинен бути ваш глобальний тип Request, що знає про req.user
res, next) => {
    const loggedInUserId = req.user?.userID;
    ...
    try {
        const success = await (0, messageModel_1.deleteMessageFromDB)(messageID, loggedInUserId);
...
const updateMessage = async (req, // Якщо використовуєте глобальні типи, req.user буде доступний
res, next) => {
    ...
    try {
        ...
        const { oldFilename, updatedMessage } = await (0, messageModel_1.updateMessageInDB)(messageIdToUpdate, currentUserId, newText, // Може бути undefined, якщо оновлюється тільки зображення (або видаляється)
        ...
        if (newMessageFilenameAction === null && oldFilename && oldFilename.trim() !== '') {
            const safeOldFilename = path_1.default.basename(oldFilename);
            const oldImageServerPath = path_1.default.resolve(__dirname, '../../uploads/message_files', safeOldFilename);
            try {
                ...
                if (unlinkError.code === 'ENOENT') {
                    ...
                    console.warn(`[messagesController][${timestamp}] Could not delete old message image '${oldImageServerPath}':`, unlinkError.message);
                }
            }
        }
        if (updatedMessage) {
            // Надсилаємо оновлене повідомлення через WebSocket всім учасникам чату
            (0, websocketService_1.sendMessageToUser)(updatedMessage.senderID, { type: 'message_updated', payload: updatedMessage });
            (0, websocketService_1.sendMessageToUser)(updatedMessage.receiverID, { type: 'message_updated', payload: updatedMessage });
